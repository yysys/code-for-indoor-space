...
/**
 * NextPossiblePar
 * to describe the touchable partitions of a given partition within an interval
 * 
 * 
 * @author lihuan
...
public class NextPossiblePar implements Comparable<Object>{
	 ...
	 */
	public NextPossiblePar(Door possibleTroughDoor, Par possibleNextPar,
			double distanceToContinue) {
		super();
	...
	public boolean Contain(SampledPoint sampledPoint){
		...
		double dist = this.possibleTroughDoor.eDist(new Point(sampledPoint.getSampledX(), sampledPoint.getSampledY(), sampledPoint.getmFloor()));
		//if(sampledPoint.getSpID() == 15633){System.out.println(dist + "," + this.distanceToContinue);}
	...
	public Rect getMBR() {
		...
		/*
		if(this.possibleTroughDoor.getX() == this.possibleNextPar.getX1()){
			// from left side
			double xBound = this.possibleTroughDoor.getX() + this.distanceToContinue;
			double yUpperBound = this.possibleTroughDoor.getY() + this.distanceToContinue;
			double yLowerBound = this.possibleTroughDoor.getY() - this.distanceToContinue;
			
			Rect rect = new Rect(this.possibleTroughDoor.getX(), xBound, yLowerBound, yUpperBound);
			return this.possibleNextPar.intersection(rect);
		}
		
		if(this.possibleTroughDoor.getX() == this.possibleNextPar.getX2()){
			// from right side
			double xBound = this.possibleTroughDoor.getX() - this.distanceToContinue;
			double yUpperBound = this.possibleTroughDoor.getY() + this.distanceToContinue;
			double yLowerBound = this.possibleTroughDoor.getY() - this.distanceToContinue;
			
			Rect rect = new Rect(xBound, this.possibleTroughDoor.getX(), yLowerBound, yUpperBound);
			return this.possibleNextPar.intersection(rect);
		}
		
		if(this.possibleTroughDoor.getY() == this.possibleNextPar.getY1()){
			// from bottom side
			double yBound = this.possibleTroughDoor.getX() + this.distanceToContinue;
			double xUpperBound = this.possibleTroughDoor.getX() + this.distanceToContinue;
			double xLowerBound = this.possibleTroughDoor.getX() - this.distanceToContinue;
			
			Rect rect = new Rect(xLowerBound, xUpperBound, this.possibleTroughDoor.getY(), yBound);
			return this.possibleNextPar.intersection(rect);
		}
		
		if(this.possibleTroughDoor.getY() == this.possibleNextPar.getY2()){
			// from top side
			double yBound = this.possibleTroughDoor.getX() - this.distanceToContinue;
			double xUpperBound = this.possibleTroughDoor.getX() + this.distanceToContinue;
			double xLowerBound = this.possibleTroughDoor.getX() - this.distanceToContinue;
			
			Rect rect = new Rect(xLowerBound, xUpperBound, yBound, this.possibleTroughDoor.getY());
			return this.possibleNextPar.intersection(rect);
		}
		*/
		
		Rect rect = new Rect(this.possibleTroughDoor.getX() - this.distanceToContinue, this.possibleTroughDoor.getX() + this.distanceToContinue, this.possibleTroughDoor.getY() - this.distanceToContinue, this.possibleTroughDoor.getY() + this.distanceToContinue);
