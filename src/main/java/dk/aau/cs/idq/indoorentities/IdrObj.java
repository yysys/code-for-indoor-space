...
public class IdrObj {
	...
	private void walkOutTheStairCase(double offsetRange) {
		...
		double randomOffset = ((new Random()).nextDouble()) * offsetRange; // the
																			// distance
																			// it
																			// will
																			// walk,
	...
	private int randomDestFloor(int curFloor) {
		...
		while (destFloor == curFloor) {
			destFloor = (new Random()).nextInt(DataGenConstant.nFloor);
		}
		return destFloor;
	}
	 ...
	 * 			Only set mTruePos
	 */
	public void setmTruePosOnly(Point mTruePos) {
		this.mTruePos = mTruePos;
	}
